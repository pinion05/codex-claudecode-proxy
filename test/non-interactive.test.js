...
test("linux install ignores relative XDG_CONFIG_HOME (no writes relative to CWD)", { skip: process.platform !== "linux" }, async (t) => {
  const home = mkTmpDir("codex-claudecode-proxy-home-");
  ...
  // Units should be installed under HOME/.config, not relative to the process CWD.
  assert.equal(
    fs.existsSync(path.join(home, ".config", "systemd", "user", "cli-proxy-api-linux.service")),
    true,
    "expected linux systemd service unit to exist under HOME/.config",
  );
  assert.equal(
    fs.existsSync(path.join(cwd, ".config", "systemd", "user", "cli-proxy-api-linux.service")),
    false,
    "expected no linux systemd service unit to be written relative to CWD",
  );
});
